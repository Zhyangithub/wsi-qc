...
def compute_score(tile):
    ...
    # QC-score: higher is better quality; penalize stripes
    score = 0.35*lv_n + 0.25*b_n + 0.25*sf_n + 0.15*(1-ss_n)
    return float(score), {"b": b, "lv": lv, "sf": sf, "ss": ss}
...
def main():
    ...
    for fname in tqdm(sorted(os.listdir(tiles_dir))):
        ...
        rows.append([x, y, score, feats["b"], feats["lv"], feats["sf"], feats["ss"]])
    with open(args.out, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["x", "y", "qc_score", "brightness", "laplacian_var", "sat_frac", "stripe_score"])
        w.writerows(rows)
