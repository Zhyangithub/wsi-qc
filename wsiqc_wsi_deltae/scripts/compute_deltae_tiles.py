...
python -m scripts.compute_deltae_tiles \
  ...
  --out_csv    D:\wsi-qc\results\example_norm\deltae_tiles.csv \
...
from src.utils.filename_parser import parse_xy_from_name
...
def main():
    ...
    for p in paths:
        ...
        try:
            ...
            rows.append((name, x, y, dE_mean, dE_median, dE_p95, dE_max))
        except Exception as e:
            print(f"[warn] {name}: {e}")

    # Write CSV
    import pandas as pd
    df = pd.DataFrame(rows, columns=["filename","x","y","deltae_mean","deltae_median","deltae_p95","deltae_max"])
